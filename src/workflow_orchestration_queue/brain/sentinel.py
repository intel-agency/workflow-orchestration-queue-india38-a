...
from workflow_orchestration_queue.brain.orchestrator import Orchestrator
from workflow_orchestration_queue.config.settings import settings
from workflow_orchestration_queue.state.models.work_item import WorkItem, WorkItemStatus
...
SENTINEL_BOT_LOGIN = settings.sentinel_bot_login
...
class Sentinel:
    """Sentinel orchestrator managing task lifecycle."""

    def __init__(self, queue: GitHubQueue) -> None:
        """Initialize the Sentinel.

        Args:
            queue: The GitHub queue to use for task management
        """
        self.queue = queue
    ...
    async def process_task(self, item: WorkItem) -> None:
        ...
        try:
            ...
            instruction = self.orchestrator.build_instruction(item)
            ...
            if res_prompt.returncode == 0:
                workflow = self.orchestrator.get_workflow(item.task_type)
                success_msg = (
                    f"✅ **Workflow Complete**\n"
    ...
    async def run_forever(self) -> None:
        """Run the sentinel polling loop forever."""
...
async def _main() -> None:
    ...
    env_values = [os.getenv(v) for v in required]
    missing = [v for v, val in zip(required, env_values, strict=False) if not val]
    if missing:
...
def run() -> None:
    """Run the sentinel (entry point for script)."""
    try:
        asyncio.run(_main())
