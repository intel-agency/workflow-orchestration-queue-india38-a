...
from workflow_orchestration_queue.state.models.work_item import (
    TaskType,
    WorkItem,
    WorkItemStatus,
    scrub_secrets,
...
class GitHubQueue(ITaskQueue):
    ...
    async def update_status(
        ...
        resp = await self.client.delete(f"{url_labels}/{WorkItemStatus.IN_PROGRESS.value}")
        if resp.status_code not in (200, 204, 404, 410):
            logger.error(f"Label cleanup failed: {resp.status_code}")

        await self.client.post(url_labels, json={"labels": [status.value]})

        if comment:
            safe_comment = scrub_secrets(comment)
            comment_url = f"{base}/issues/{item.issue_number}/comments"
            await self.client.post(comment_url, json={"body": safe_comment})
    ...
    async def claim_task(
        ...
        resp = await self.client.delete(f"{url_labels}/{WorkItemStatus.QUEUED.value}")
        if resp.status_code not in (200, 204, 404, 410):
        ...
        await self.client.post(comment_url, json={"body": msg})
